...
downloadButton.addEventListener('click', async () => {
  ...
  try {
    ...
    const originalMostVisibility = Array.from(mostButtons).map(btn => btn.style.visibility);
    const originalLikeVisibility = Array.from(likeButtons).map(btn => btn.style.visibility);
    ...
    const canvas = await html2canvas(container, {
      ...
      onclone: (clonedDoc) => {
        ...
        // Показываем активные кнопки в PDF
        clonedDoc.querySelectorAll('.job.green .most-btn').forEach(btn => {
          btn.style.visibility = 'visible';
        });
        
        clonedDoc.querySelectorAll('.education-item.liked .like-btn').forEach(btn => {
          btn.style.visibility = 'visible';
        });
      }
    });
